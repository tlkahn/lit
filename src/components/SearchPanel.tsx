...
function ActiveFilterChips() {
  ...
  if (filter.mtime_after || filter.mtime_before) {
    const parts: string[] = [];
    if (filter.mtime_after) parts.push(`after ${new Date(filter.mtime_after * 1000).toLocaleDateString()}`);
    if (filter.mtime_before) parts.push(`before ${new Date(filter.mtime_before * 1000).toLocaleDateString()}`);
    chips.push({
      key: "date",
...
export function SearchPanel() {
  ...
  // Listen for focus-search-panel event
  useEffect(() => {
    const handler = () => {
      inputRef.current?.focus();

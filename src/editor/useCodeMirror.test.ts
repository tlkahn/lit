...
describe("useCodeMirror", () => {
  ...
  it("editor becomes non-editable when llmLocked is set", () => {
  ...
  it("editor becomes editable again when llmLocked is cleared", () => {
    useModalLockStore.setState({ openCount: 0, locked: false, llmLocked: true });
    const { result } = renderHook(() =>
      useCodeMirror({
        containerRef: container.ref,
        doc: "hello",
  ...
  it("both locks compose: locked + llmLocked", () => {
    useModalLockStore.setState({ openCount: 0, locked: false, llmLocked: false });
    const { result } = renderHook(() =>
      useCodeMirror({
        containerRef: container.ref,
    ...
    act(() => { useModalLockStore.getState().increment(); });
    expect(view.state.facet(EditorView.editable)).toBe(false);

    act(() => { useModalLockStore.getState().setLlmLocked(true); });
    expect(view.state.facet(EditorView.editable)).toBe(false);

    act(() => { useModalLockStore.getState().decrement(); });
    expect(view.state.facet(EditorView.editable)).toBe(false);

    act(() => { useModalLockStore.getState().setLlmLocked(false); });
    expect(view.state.facet(EditorView.editable)).toBe(true);
  });

  it("initial state respects llmLocked", () => {
    useModalLockStore.setState({ openCount: 0, locked: false, llmLocked: true });
    const { result } = renderHook(() =>
      useCodeMirror({
        containerRef: container.ref,

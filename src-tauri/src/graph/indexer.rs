    ...
    for (i, node) in all_nodes.iter().enumerate() {
        ...
        // Always call upsert even with empty vec so orphaned annotations get cleaned up.
        {
            let anns = if annotations_enabled {
                super::extract::extract_annotations(&node.body, &mark_codes)
            } else {
                vec![]
            };
        ...
        for cite in extract_citations_blanked(blanked, &node.body) {
...
pub fn incremental_reindex(
    ...
    for path in diff.new.iter().chain(diff.changed.iter()) {
        match parse_md_file(root, path) {
            Ok((node, links, blanked)) => {
                ...
                for link in &links {
                    let resolved = stem_lookup.resolve(&link.target);
                    let (context, source_line) = extract_sentence_context(&node.body, &link.target);
                    let target_id = match &resolved.node_id {
                        Some(id) => id.clone(),
                        None => {
    ...
    if !changed_stems.is_empty() {
        ...
        for source_id in &affected {
            ...
            if let Ok((node, links, blanked)) = parse_md_file(root, source_id) {
...
mod tests {
    ...
    fn parse_md_file_returns_body() {
        ...
        let (node, _, _) = parse_md_file(dir.path(), "note.md").unwrap();
        assert_eq!(node.body, "The body text.\n\nSecond paragraph.");
    }
    ...
    fn parse_md_file_returns_body_no_frontmatter() {
        ...
        let (node, _, _) = parse_md_file(dir.path(), "plain.md").unwrap();
        assert_eq!(node.body, "Just plain text.");
    }
    ...
    fn parse_md_file_returns_blanked_body() {
        ...
        let (node, _, blanked) = parse_md_file(dir.path(), "note.md").unwrap();
        assert_eq!(blanked, blank_code(&node.body));
    }

...
mod tests {
    ...
    #[test]
    fn full_subgraph_default_excludes_cardbox_edges() {
        let store = Store::open_memory().unwrap();
        store.upsert_node(&make_node("A", "Alpha"), 1).unwrap();
        store.upsert_node(&make_node("B", "Beta"), 1).unwrap();
        store.upsert_node(&make_node("C", "Charlie"), 1).unwrap();
        store.insert_edge("A", "B", "wiki", "b", 0, EdgeKind::Wikilink).unwrap();
        store.insert_edge("A", "C", "box", "c", 0, EdgeKind::Cardbox).unwrap();
        let kg = KnowledgeGraph::from_store(&store).unwrap();

        let result = kg.full_subgraph_filtered(false, false);
        assert!(
            result.edges.iter().all(|(_, _, k)| *k == EdgeKind::Wikilink),
            "cardbox edges should be excluded by default, got: {:?}",
            result.edges
        );
    ...
    #[test]
    fn full_subgraph_include_cardbox_shows_cardbox_edges() {
        let store = Store::open_memory().unwrap();
        store.upsert_node(&make_node("A", "Alpha"), 1).unwrap();
        store.upsert_node(&make_node("B", "Beta"), 1).unwrap();
        store.upsert_node(&make_node("C", "Charlie"), 1).unwrap();
        ...
        assert!(
            result.edges.iter().any(|(_, _, k)| *k == EdgeKind::Cardbox),
            "cardbox edges should be present when include_cardbox=true"
        );
        assert!(
            result.edges.iter().any(|(_, _, k)| *k == EdgeKind::Wikilink),
            "wikilink edges should still be present"
        );
    }

    #[test]
    fn subgraph_default_excludes_cardbox_edges() {
        let store = Store::open_memory().unwrap();
        store.upsert_node(&make_node("A", "Alpha"), 1).unwrap();
        store.upsert_node(&make_node("B", "Beta"), 1).unwrap();
        store.upsert_node(&make_node("C", "Charlie"), 1).unwrap();
        store.insert_edge("A", "B", "wiki", "b", 0, EdgeKind::Wikilink).unwrap();
        store.insert_edge("B", "C", "box", "c", 0, EdgeKind::Cardbox).unwrap();
        let kg = KnowledgeGraph::from_store(&store).unwrap();

        let result = kg.subgraph_filtered(&["A"], 1, true, false, false).unwrap();
        let ids: HashSet<&str> = result.nodes.iter().map(|n| n.id.as_str()).collect();
        assert!(!ids.contains("C"), "cardbox-adjacent node should not appear");
        assert!(
            result.edges.iter().all(|(_, _, k)| *k == EdgeKind::Wikilink),
            "no cardbox edges when include_cardbox=false"
        );
    }

    #[test]
    fn subgraph_include_cardbox_shows_cardbox_edges() {
        let store = Store::open_memory().unwrap();
        store.upsert_node(&make_node("A", "Alpha"), 1).unwrap();
        store.upsert_node(&make_node("B", "Beta"), 1).unwrap();
        store.upsert_node(&make_node("C", "Charlie"), 1).unwrap();
        ...
        let result = kg.subgraph_filtered(&["A"], 1, true, false, true).unwrap();
        let ids: HashSet<&str> = result.nodes.iter().map(|n| n.id.as_str()).collect();
        assert!(ids.contains("C"), "cardbox-adjacent materialized node should appear");
        assert!(
            result.edges.iter().any(|(_, _, k)| *k == EdgeKind::Cardbox),
            "cardbox edges should be present when include_cardbox=true"
        );
    ...
    #[test]
    fn subgraph_include_both_citations_and_cardbox() {
        let store = Store::open_memory().unwrap();
        store.upsert_node(&make_node("A", "Alpha"), 1).unwrap();
        store.upsert_node(&make_node("B", "Beta"), 1).unwrap();
        store.upsert_shadow("bib:ref1", "Ref (2024)", Materialization::Shadow).unwrap();
        ...
        let result = kg.subgraph_filtered(&["A"], 1, true, true, true).unwrap();
        let ids: HashSet<&str> = result.nodes.iter().map(|n| n.id.as_str()).collect();
        assert!(ids.contains("bib:ref1"), "citation shadow should appear");
        assert!(ids.contains("C"), "cardbox-adjacent node should appear");
        assert!(
            result.edges.iter().any(|(_, _, k)| *k == EdgeKind::Citation),
            "citation edges present"
        );
        assert!(
            result.edges.iter().any(|(_, _, k)| *k == EdgeKind::Cardbox),
            "cardbox edges present"
        );
        assert!(
            result.edges.iter().any(|(_, _, k)| *k == EdgeKind::Wikilink),
            "wikilink edges present"
        );
    }

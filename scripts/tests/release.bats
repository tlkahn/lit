...
# The patch must be scoped to the `lit` package block only — other packages that
# happen to share the old version string must be left untouched.

@test "set-version.sh: bumps the lit package version in Cargo.lock, scoped to lit only" {
  REPO_ROOT="$TEST_TEMP_DIR"
  mkdir -p "$TEST_TEMP_DIR/src-tauri"
  echo '{"version":"0.0.0"}' > "$TEST_TEMP_DIR/package.json"
  echo '{"version":"0.0.0"}' > "$TEST_TEMP_DIR/src-tauri/tauri.conf.json"
...
foo = "0.0.0"
TOML
  cat > "$TEST_TEMP_DIR/src-tauri/Cargo.lock" <<'LOCK'
...
LOCK
  ...
  # The lit package block is bumped.
  run awk '/^name = "lit"$/{getline; print; exit}' "$TEST_TEMP_DIR/src-tauri/Cargo.lock"
  [ "$output" = 'version = "0.13.0"' ]

  # The sibling package (litemap) sharing the old version is untouched.
  run awk '/^name = "litemap"$/{getline; print; exit}' "$TEST_TEMP_DIR/src-tauri/Cargo.lock"
